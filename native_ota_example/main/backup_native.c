...
static void ota_example_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (data_read < 0) {
            ...
            unsigned char outputlast[580] = {0};
            char hash[33] = {0};
            ...
            if (memcmp( hmac , Mac,32) == 0 )
            {
                ...
                if ( IntIndex == j && j == 0 ) {
                    ...
                    if (memcmp( Hash , h0,32) == 0 ){
                         ...
                         if (image_header_was_checked == false)
                         {
                             ...
                             if (output > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t))
                             {
                         ...
                         err = esp_ota_write(update_handle, (const void *)output, 956);
                         if (err != ESP_OK)
                         {
                             http_cleanup(client);
                             task_fatal_error();
                ...
                else if (IntIndex == j)
                {
                    ...
                    if (memcmp( Hash , previous_Hash,32 ) == 0)
                    {
                        ...
                        if (IntIndex == (N-1))
                        {
                            decrypt_symmetric((unsigned char *)Enc, IV, outputlast);
                            //write ota_write_data in memory
                            err = esp_ota_write(update_handle, (const void *)outputlast, 580);
                        }else
                        {
                            decrypt_symmetric((unsigned char *)Enc, IV, output);
                            //write ota_write_data in memory
